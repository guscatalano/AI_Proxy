...
# coding-v3 = every coding-v2 task plus the wave below: three new grading
# languages (SQL on stdlib sqlite, bash, Go) and harder traps in the existing ones.
# The stability contract applies from the moment this ships.
_V3_EXTRA: list[dict] = [
    {
        "id": 'quoted_split',
        "lang": 'python',
        "tier": 'hard',
        "entry": 'quoted_split',
        "prompt": 'Write a Python function `quoted_split(s: str) -> list | None` splitting s into tokens on\nspaces/tabs. A double-quoted section groups into one token (quotes removed). Inside quotes,\nbackslash escapes `\\"` and `\\\\` produce a literal quote/backslash. `""` is an empty token.\nAn unterminated quote returns None. Return only the function in a single python code block.',
        "cases": [{'args': ['a b c'], 'expect': ['a', 'b', 'c']}, {'args': ['a "b c" d'], 'expect': ['a', 'b c', 'd']}, {'args': ['"a\\"b"'], 'expect': ['a"b']}, {'args': ['  a   b '], 'expect': ['a', 'b']}, {'args': ['a "unclosed'], 'expect': None}, {'args': ['""'], 'expect': ['']}, {'args': ['say "hi there" now'], 'expect': ['say', 'hi there', 'now']}],
    },
    {
        "id": 'ring_buffer',
        "lang": 'python',
        "tier": 'hard',
        "entry": 'ring',
        "prompt": "Write a Python function `ring(capacity: int, ops: list) -> list` simulating a ring buffer\nof fixed capacity. Each op is ['push', value] or ['pop']. A push onto a FULL buffer silently\noverwrites the OLDEST element. Pop removes and returns the oldest element, or None when empty.\nReturn the list of pop results, in order. Return only the function in a single python code\nblock.",
        "cases": [{'args': [2, [['push', 1], ['push', 2], ['push', 3], ['pop'], ['pop'], ['pop']]], 'expect': [2, 3, None]}, {'args': [1, [['push', 5], ['push', 6], ['pop'], ['pop']]], 'expect': [6, None]}, {'args': [3, [['pop'], ['push', 1], ['pop'], ['pop']]], 'expect': [None, 1, None]}, {'args': [2, [['push', 1], ['pop'], ['push', 2], ['push', 3], ['push', 4], ['pop'], ['pop']]], 'expect': [1, 3, 4]}],
    },
    {
        "id": 'expand_ranges',
        "lang": 'python',
        "tier": 'hard',
        "entry": 'expand_ranges',
        "prompt": 'Write a Python function `expand_ranges(s: str) -> list | None` expanding a spec like\n"1-3,7,10-12" into a sorted list of unique ints. Whitespace around numbers, hyphens and commas\nis allowed. Overlapping ranges merge. Invalid input — empty string, empty part, non-numeric,\nor a reversed range like "5-3" — returns None. Return only the function in a single python\ncode block.',
        "cases": [{'args': ['1-3,7'], 'expect': [1, 2, 3, 7]}, {'args': ['3-3'], 'expect': [3]}, {'args': ['5-3'], 'expect': None}, {'args': ['1-3,2-4'], 'expect': [1, 2, 3, 4]}, {'args': ['a-b'], 'expect': None}, {'args': [''], 'expect': None}, {'args': [' 1 - 3 , 5'], 'expect': [1, 2, 3, 5]}, {'args': ['9,1,4-5'], 'expect': [1, 4, 5, 9]}, {'args': ['1,,2'], 'expect': None}],
    },
    {
        "id": 'dedent_text',
        "lang": 'python',
        "tier": 'hard',
        "entry": 'dedent_text',
        "prompt": 'Write a Python function `dedent_text(s: str) -> str` removing the longest common leading\nwhitespace prefix (matched literally, character by character — tabs and spaces are different\ncharacters) from every line. Lines that are empty or whitespace-only are ignored when\ncomputing the prefix and become empty strings in the output. Line count is preserved; lines\nare separated by \\n. Return only the function in a single python code block.',
        "cases": [{'args': ['    a\n      b\n    c'], 'expect': 'a\n  b\nc'}, {'args': ['\ta\n\t\tb'], 'expect': 'a\n\tb'}, {'args': ['    a\n\tb'], 'expect': '    a\n\tb'}, {'args': ['  a\n\n  b'], 'expect': 'a\n\nb'}, {'args': ['   \n  a\n   b'], 'expect': '\na\n b'}, {'args': ['a\n  b'], 'expect': 'a\n  b'}],
    },
    {
        "id": 'deep_get',
        ...
        "tier": 'hard',
        "entry": 'deepGet',
        "prompt": 'Write a JavaScript function `deepGet(obj, path)` resolving a path like "a.b[2].c":\ndot-separated keys, with [n] suffixes indexing arrays only (an [n] on a non-array is a miss).\nAny missing key, out-of-range index, or an empty path returns null. A stored null value is\nreturned as null. Return only the function in a single javascript code block.',
        "cases": [{'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, 'a.b[1].c'], 'expect': 7}, {'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, 'a.b[2].c'], 'expect': None}, {'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, 'x[1][0]'], 'expect': 2}, {'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, 'a.z'], 'expect': None}, {'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, ''], 'expect': None}, {'args': [{'a': {'1': 9}}, 'a[1]'], 'expect': None}, {'args': [{'a': {'b': [{'c': 5}, {'c': 7}]}, 'x': [1, [2, 3]], 'n': None}, 'n'], 'expect': None}],
    },
    {
        "id": 'sql_top_spenders',
        "lang": 'sql',
        "tier": 'hard',
        "entry": 'query',
        "prompt": "Write a single SQLite SELECT for a table `orders(customer TEXT, amount INTEGER)`:\nreturn each customer whose ORDER TOTAL (sum of all their amounts) is at least 100, as two\ncolumns (customer, total), ordered by total descending then customer ascending. Note the\nthreshold applies to the customer's total, not to individual orders. Return only the SQL in a\nsingle sql code block.",
        "cases": [{'setup': "CREATE TABLE orders (customer TEXT, amount INTEGER);INSERT INTO orders VALUES ('ada',60),('ada',50),('bob',99),('cy',150);", 'expect': [['cy', 150], ['ada', 110]]}, {'setup': "CREATE TABLE orders (customer TEXT, amount INTEGER);INSERT INTO orders VALUES ('ada',100);INSERT INTO orders VALUES ('bob',40),('bob',60);INSERT INTO orders VALUES ('cy',99),('cy',1);", 'expect': [['ada', 100], ['bob', 100], ['cy', 100]]}, {'setup': "CREATE TABLE orders (customer TEXT, amount INTEGER);INSERT INTO orders VALUES ('zed',300),('amy',300),('bob',20),('bob',85);", 'expect': [['amy', 300], ['zed', 300], ['bob', 105]]}, {'setup': "CREATE TABLE orders (customer TEXT, amount INTEGER);INSERT INTO orders VALUES ('solo',99);", 'expect': []}],
    },
    {
        "id": 'sql_missing_users',
        "lang": 'sql',
        "tier": 'hard',
        "entry": 'query',
        "prompt": 'Write a single SQLite SELECT for tables `users(id INTEGER, name TEXT)` and\n`orders(user_id INTEGER)`: return the names of users who have NO orders, ordered by name\nascending, one column. Careful: orders.user_id can be NULL — a `NOT IN` over a set containing\nNULL matches nothing. Return only the SQL in a single sql code block.',
        "cases": [{'setup': "CREATE TABLE users (id INTEGER, name TEXT);CREATE TABLE orders (user_id INTEGER);INSERT INTO users VALUES (1,'ada'),(2,'bob'),(3,'cy');INSERT INTO orders VALUES (1),(NULL);", 'expect': [['bob'], ['cy']]}, {'setup': "CREATE TABLE users (id INTEGER, name TEXT);CREATE TABLE orders (user_id INTEGER);INSERT INTO users VALUES (1,'ada'),(2,'bob');INSERT INTO orders VALUES (1),(2);", 'expect': []}, {'setup': "CREATE TABLE users (id INTEGER, name TEXT);CREATE TABLE orders (user_id INTEGER);INSERT INTO users VALUES (1,'zed'),(2,'amy');INSERT INTO orders VALUES (NULL);", 'expect': [['amy'], ['zed']]}, {'setup': "CREATE TABLE users (id INTEGER, name TEXT);CREATE TABLE orders (user_id INTEGER);INSERT INTO users VALUES (5,'eve'),(6,'dan'),(7,'ana');INSERT INTO orders VALUES (6),(NULL),(6);", 'expect': [['ana'], ['eve']]}],
    },
    {
        "id": 'bash_dedup_lines',
        "lang": 'bash',
        "tier": 'hard',
        "entry": 'script',
        "prompt": 'Write a bash script (it will be run with the input on stdin) that prints each DISTINCT\ninput line once, keeping only the FIRST occurrence and preserving the original order. Note\n`sort -u` reorders, so it cannot be the answer. Return only the script in a single bash code\nblock.',
        "cases": [{'stdin': 'b\na\nb\nc\na\n', 'expect': 'b\na\nc'}, {'stdin': 'x\nx\nx\n', 'expect': 'x'}, {'stdin': 'one\n', 'expect': 'one'}, {'stdin': 'beta\nalpha\nbeta\nALPHA\n', 'expect': 'beta\nalpha\nALPHA'}],
    },
    {
        "id": 'bash_sort_versions',
        "lang": 'bash',
        "tier": 'hard',
        "entry": 'script',
        "prompt": 'Write a bash script (input on stdin) that sorts version strings like "1.9.0" one per\nline in ASCENDING order, comparing each dot-separated component NUMERICALLY — so 1.9.0 comes\nbefore 1.10.0. Plain lexicographic sort gets that pair wrong. Return only the script in a\nsingle bash code block.',
        "cases": [{'stdin': '1.10.0\n1.9.0\n1.2.10\n', 'expect': '1.2.10\n1.9.0\n1.10.0'}, {'stdin': '2.0.0\n10.0.0\n9.9.9\n', 'expect': '2.0.0\n9.9.9\n10.0.0'}, {'stdin': '0.0.1\n0.0.1\n0.1.0\n', 'expect': '0.0.1\n0.0.1\n0.1.0'}, {'stdin': '3.0.12\n3.0.2\n3.0.1\n', 'expect': '3.0.1\n3.0.2\n3.0.12'}],
    },
    {
        "id": 'go_rle_decode',
        ...
        "tier": 'hard',
        "entry": 'RleDecode',
        "prompt": 'Write a Go function `func RleDecode(s string) string` decoding run-length pairs: each\nletter is followed by its count in decimal digits (count >= 1, may be multi-digit), e.g.\n"a3b2" -> "aaabb" and "a12" is 12 a\'s. Any malformed input — empty count, a count with a\nleading zero, a digit where a letter belongs, or an empty final count — returns "". The empty\nstring decodes to "". Return only the function in a single go code block.',
        "cases": [{'args': ['a3b2'], 'expect': 'aaabb'}, {'args': ['z1'], 'expect': 'z'}, {'args': [''], 'expect': ''}, {'args': ['ab'], 'expect': ''}, {'args': ['3a'], 'expect': ''}, {'args': ['a0'], 'expect': ''}, {'args': ['a12'], 'expect': 'aaaaaaaaaaaa'}, {'args': ['a3b2c1'], 'expect': 'aaabbc'}],
    },
    {
        "id": 'go_ipv4_valid',
        "lang": 'go',
        "tier": 'hard',
        "entry": 'IPv4Valid',
        "prompt": 'Write a Go function `func IPv4Valid(s string) bool` validating a strict dotted-quad\nIPv4 address: exactly four parts separated by dots, each all digits, each 0-255, and no\nleading zeros ("01" is invalid, "0" is valid). Return only the function in a single go code\nblock.',
        "cases": [{'args': ['192.168.0.1'], 'expect': True}, {'args': ['0.0.0.0'], 'expect': True}, {'args': ['256.1.1.1'], 'expect': False}, {'args': ['01.2.3.4'], 'expect': False}, {'args': ['1.2.3'], 'expect': False}, {'args': ['1.2.3.4.5'], 'expect': False}, {'args': ['a.b.c.d'], 'expect': False}, {'args': ['255.255.255.255'], 'expect': True}, {'args': ['1.2.-3.4'], 'expect': False}],
    },
    {
        "id": 'c_bit_count_range',
        "lang": 'c',
        "tier": 'hard',
        "entry": 'bit_count_range',
        "prompt": 'Write a C function `long long bit_count_range(int a, int b)` returning the total number\nof set bits across every integer i with a <= i <= b, where 0 <= a <= b <= INT_MAX. Careful:\na loop like `for (int i = a; i <= b; i++)` never terminates when b == INT_MAX — the increment\noverflows. Return only the function in a single c code block.',
        "cases": [{'args': [0, 0], 'expect': 0}, {'args': [0, 7], 'expect': 12}, {'args': [5, 5], 'expect': 2}, {'args': [1, 100], 'expect': 319}, {'args': [2147483645, 2147483647], 'expect': 91}],
    },
    {
        "id": 'cpp_wrap_count',
        "lang": 'cpp',
        "tier": 'hard',
        "entry": 'wrap_count',
        "prompt": 'Write a C++ function `int wrap_count(const std::string& s, int width)` returning how\nmany lines greedy word-wrapping produces: words are separated by single spaces, each line\nholds as many words as fit in `width` characters counting single joining spaces, and a word\nLONGER than width is placed alone on its own line, never split. An empty string is 0 lines.\nReturn only the function in a single cpp code block.',
        "cases": [{'args': ['the quick brown fox', 10], 'expect': 2}, {'args': ['a bb ccc', 3], 'expect': 3}, {'args': ['supercalifragilistic word', 10], 'expect': 2}, {'args': ['', 5], 'expect': 0}, {'args': ['one', 80], 'expect': 1}, {'args': ['aa bb', 5], 'expect': 1}],
    },
    {
        "id": 'rust_kv_get',
        "lang": 'rust',
        "tier": 'hard',
        "entry": 'kv_get',
        "prompt": 'Write a Rust function `fn kv_get(s: &str, key: &str) -> String` over entries separated\nby \';\'. Each entry splits on its FIRST \'=\' into key and value (so "a=b=c" has value "b=c").\nEntries without \'=\' are ignored. If the key appears more than once the LAST value wins; a\nmissing key returns the empty string. Return only the function in a single rust code\nblock.',
        "cases": [{'args': ['a=1;b=2', 'b'], 'expect': '2'}, {'args': ['a=1;a=2', 'a'], 'expect': '2'}, {'args': ['a=b=c;d=e', 'a'], 'expect': 'b=c'}, {'args': ['x=1;y', 'y'], 'expect': ''}, {'args': ['', 'a'], 'expect': ''}, {'args': ['k=;k2=v', 'k'], 'expect': ''}, {'args': ['a=1', 'missing'], 'expect': ''}],
    },
    {
        "id": 'cs_round_half',
        "lang": 'csharp',
        "tier": 'hard',
        "entry": 'Sol.RoundHalf',
        "prompt": "Write a C# static class `Sol` containing exactly one method,\n`public static int RoundHalf(double v)`, rounding to the nearest integer with halves rounding\nAWAY FROM ZERO: 2.5 -> 3, -2.5 -> -3. Careful: C#'s Math.Round defaults to banker's rounding,\nwhich rounds 2.5 to 2. Return only the class in a single csharp code block.",
        "cases": [{'args': [2.5], 'expect': 3}, {'args': [3.5], 'expect': 4}, {'args': [-2.5], 'expect': -3}, {'args': [2.4], 'expect': 2}, {'args': [-0.5], 'expect': -1}, {'args': [0.0], 'expect': 0}, {'args': [7.5], 'expect': 8}, {'args': [-3.6], 'expect': -4}],
    },
    {
        "id": 'php_flatten_keys',
        "lang": 'php',
        "tier": 'hard',
        "entry": 'flatten_keys',
        "prompt": "Write a PHP function `flatten_keys(array $a): array` flattening a nested associative\narray into a single level whose keys join the path with '.': ['a'=>['b'=>1]] becomes\n['a.b'=>1]. Non-array values (including null and false) are kept as-is. Return only the\nfunction in a single php code block.",
        "cases": [{'args': [{'a': {'b': 1, 'c': {'d': 2}}, 'e': 3}], 'expect': {'a.b': 1, 'a.c.d': 2, 'e': 3}}, {'args': [{'x': {'y': {'z': 'deep'}}}], 'expect': {'x.y.z': 'deep'}}, {'args': [{'k': 1}], 'expect': {'k': 1}}, {'args': [{'a': {'b': None}, 'c': False}], 'expect': {'a.b': None, 'c': False}}],
    },
    {
        "id": 'html_nav_current',
        "lang": 'html',
        "tier": 'hard',
        "entry": 'nav',
        "prompt": 'Write an HTML fragment (no document shell): a <nav> containing a <ul> of exactly three\n<li> items, each holding an <a>. The links point to /, /about and /contact. The About link —\nand only it — carries aria-current="page" to mark the current page. Return only the fragment\nin a single html code block.',
        "cases": [{'op': 'count', 'sel': 'nav', 'expect': 1}, {'op': 'count', 'sel': 'nav ul li a', 'expect': 3}, {'op': 'attr', 'sel': 'a[aria-current=page]', 'name': 'href', 'expect': '/about'}, {'op': 'count', 'sel': 'a[aria-current]', 'expect': 1}],
    },
    {
        "id": 'css_sticky_footer',
        "lang": 'css',
        "tier": 'hard',
        "entry": 'sticky',
        "prompt": 'Write CSS for a sticky footer using flexbox — no absolute or fixed positioning: `body`\nis a flex column filling at least the full viewport height (100vh), and `footer` pushes itself\nto the bottom with an automatic top margin. Return only the CSS in a single css code\nblock.',
        "cases": [{'sel': 'body', 'prop': 'display', 'expect': 'flex'}, {'sel': 'body', 'prop': 'flex-direction', 'expect': 'column'}, {'sel': 'body', 'prop': 'min-height', 'expect': '100vh'}, {'sel': 'footer', 'prop': 'margin-top', 'expect': 'auto'}],
    },
]

SUITES["coding-v3"] = SUITES["coding-v2"] + _V3_EXTRA
...
TASK_DESC.update({
    ...
    'deep_get': 'Path lookup like a.b[2].c, null on any miss',
    'sql_top_spenders': 'GROUP BY total with HAVING — not a WHERE',
    'sql_missing_users': 'Anti-join where NOT IN meets a NULL',
    'bash_dedup_lines': 'First occurrence wins, order preserved (sort -u reorders)',
    'bash_sort_versions': 'Numeric per-component version sort',
    'go_rle_decode': 'Decode letter+count runs, strict validation',
    'go_ipv4_valid': 'Strict dotted-quad IPv4 — leading zeros invalid',
    'c_bit_count_range': 'Popcount over [a,b] — i <= INT_MAX loop overflow trap',
    'cpp_wrap_count': 'Greedy word-wrap line count — long words never split',
    'rust_kv_get': "k=v parsing: first '=' splits, last duplicate wins",
    'cs_round_half': "Half away from zero — Math.Round is banker's",
    'php_flatten_keys': 'Nested assoc array to dot-notation keys',
